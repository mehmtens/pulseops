...
describe('App', () => {
  ...
  it('shows reliability controls on the dashboard', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Members and invitations' })).toBeInTheDocument()
    expect(screen.getByLabelText('Escalation delay (seconds)')).toBeInTheDocument()
    expect(screen.getByRole('group', { name: 'Notification channels' })).toBeInTheDocument()
    expect(screen.getAllByRole('option', { name: 'Admin' })).toHaveLength(2)
