...
type Monitor = {
  ...
  monitorType: 'http' | 'heartbeat'; expectedKeyword: string
}
type MonitorForm = { name: string; monitorType: 'http' | 'heartbeat'; url: string; expectedKeyword: string; intervalSeconds: number; timeoutSeconds: number; failureThreshold: number; recoveryThreshold: number; maintenanceUntil: string; active: boolean; public: boolean }
...
type ApiKey = { id: number; name: string; prefix: string; scope: 'read' | 'write'; lastUsedAt: string | null; createdAt: string }
...
export function App() {
  ...
  const [newKey, setNewKey] = useState({ name: '', scope: 'read' as 'read' | 'write' })
  ...
  return <div className="shell dashboard">
    ...
    <main>
      ...
      <section className="panel form-panel"><div><p className="kicker">{editing ? 'EDIT MONITOR' : 'NEW MONITOR'}</p><h2>{editing ? 'Update endpoint' : 'Watch an endpoint'}</h2></div><form onSubmit={save}>
        ...
        <label>Monitor type<select value={form.monitorType} disabled={editing !== null} onChange={(e) => setForm({ ...form, monitorType: e.target.value as 'http' | 'heartbeat' })}><option value="http">HTTP / HTTPS</option><option value="heartbeat">Cron heartbeat</option></select></label>
      ...
      <section className="panel keys"><div><p className="kicker">ACCESS</p><h2>API keys</h2><p>Read keys can inspect data. Write keys can also manage monitors and incidents.</p></div><form onSubmit={createKey}><label>Name<input value={newKey.name} maxLength={100} onChange={(e) => setNewKey({ ...newKey, name: e.target.value })} placeholder="Dashboard integration" required /></label><label>Scope<select value={newKey.scope} onChange={(e) => setNewKey({ ...newKey, scope: e.target.value as 'read' | 'write' })}><option value="read">Read only</option><option value="write">Read and write</option></select></label><button>Create key</button></form>{createdToken && <p className="token-result"><strong>Copy now — shown once:</strong> <code>{createdToken}</code></p>}<div className="key-list">{keys.map((key) => <div key={key.id}><span><strong>{key.name}</strong><small>{key.prefix}… · {key.scope} · {key.lastUsedAt ? `used ${date(key.lastUsedAt)}` : 'never used'}</small></span><button className="danger compact" onClick={() => deleteKey(key)}>Revoke</button></div>)}</div></section>
    </main>
